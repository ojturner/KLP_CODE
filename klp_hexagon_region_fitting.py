...
def fit_all_regions(sn_type,
    ...
    ###################################################################
    ###################################################################
    ############### CREATING THE HEXAGON REGIONS ######################
    ###################################################################
    ###################################################################

    # the creation of the hexagon regions will be done in a separate
    # method. It depends on the chosen sn_type and will return a list
    # of masks which will then be cycled through in an identical fashion
    # to the cycling through of the ds9 regions

    yj_mask_list = create_hexagon_mask_list(sn_type,
                                            yj_cube_data.shape[1],
                                            yj_cube_data.shape[2],
                                            yj_centre_x,
                                            yj_centre_y)
    h_mask_list = create_hexagon_mask_list(sn_type,
                                            h_cube_data.shape[1],
                                            h_cube_data.shape[2],
                                            h_centre_x,
                                            h_centre_y)
    k_mask_list = create_hexagon_mask_list(sn_type,
                                            k_cube_data.shape[1],
                                            k_cube_data.shape[2],
                                            k_centre_x,
                                            k_centre_y)

    # beginning of big loop over all hexagonal segments
    # define the list to accommodate all the flux values
    all_segments_line_flux_list = []
    for yj_mask, h_mask, k_mask in zip(yj_mask_list, h_mask_list, k_mask_list):

        print 'FITTING REGION: %s' % str(plot_counter)
        ...
        # plotting the KBSS redshift 2 points
        klps_errorbar = ax.errorbar(klp_log_nii_ha[plot_counter-1],
                                    klp_log_oiii_hb[plot_counter-1],
                                    ecolor='black',
                                    xerr=np.array([[klp_lower_error_log_nii_ha[plot_counter-1],klp_upper_error_log_nii_ha[plot_counter-1]]]).T,
                                    yerr=np.array([[klp_lower_error_log_oiii_hb[plot_counter-1],klp_upper_error_log_oiii_hb[plot_counter-1]]]).T,
                                    marker='*',
                                    markersize=20,
                                    markerfacecolor='red',
                                    markeredgecolor='red',
        ...
        all_plot_save_name = plot_save_dir + 'all_' + str(plot_counter) + '_' + sn_type + '_fit.png'
        ...
        line_ax[0][0].text(yj_wave_array[yj_min_index]+0.002,
                           ...
                           fontsize=25,
                           fontweight='bold')
        line_ax[0][1].text(h_wave_array[h_min_index]+0.010,
                           0.45*y_limit,
                           r'$\textbf{H-Band}$',
                           fontsize=25,
                           fontweight='bold')
        ...
        # take the region number and sn_type and make a main title
        main_title_string = sn_type + str(plot_counter)

        line_fig.suptitle(r'$\textbf{%s}$' % main_title_string,
                          fontsize=30,
                          fontweight='bold')
...
#                            labelpad=10)
        ...
        # append to the all_flux_line_list the measured quantities
        all_segments_line_flux_list.append([[oii_total_flux,oii_total_flux_lower_error,oii_total_flux_upper_error,oii_total_sn,oii_total_sn_lower_error,oii_total_sn_upper_error,oii_sigma,oii_sigma_lower_error,oii_sigma_upper_error],
                                            ...
                                            [sn_type,plot_counter]])
    
        # finally increment the plot counter
        plot_counter += 1
    ...
    fig.tight_layout()
    # plt.show()
    fig.savefig(plot_save_dir + 'bpt_hexagons.png')
    plt.close('all')

    # return the emission line strengths, errors, sigma values
    # and errors for outputting to a table

    return all_segments_line_flux_list

def create_hexagon_mask_list(sn_type,
                             shape_x,
                             shape_y,
                             centre_x,
                             centre_y):
    ...
    """
    Def:
    Return the list of masks to apply to the data, given the sn_type
    and centre coordinates. 
    """
    # define the list which will be used to house the different masks
    mask_list = []
    # integer and round the central x and y values
    x_c = int(round(centre_x))
    y_c = int(round(centre_y))

    # now decide what to do on the basis of the S/N for that galaxy
    # low signal to noise - use the original large hexagons
    if sn_type == 'low':

        # REGION 1 - THE CENTRE
        # set the allow values array - these will be used in conjunction
        # with the mask to set some values equal to TRUE
        allow_values = []
        # construct the base mask, which we will set some values
        # equal to true to extract the spectra
        mask = np.full(shape=(shape_x,shape_y),
                       fill_value=False,
                       dtype=bool)
        # central region which is a cross
        allow_values.append([x_c,y_c])
        allow_values.append([x_c,y_c-1])
        allow_values.append([x_c-1,y_c-1])
        allow_values.append([x_c-1,y_c])
        allow_values.append([x_c-2,y_c])
        allow_values.append([x_c-2,y_c-1])
        allow_values.append([x_c-1,y_c-2])
        allow_values.append([x_c,y_c-2])
        allow_values.append([x_c+1,y_c])
        allow_values.append([x_c+1,y_c-1])
        allow_values.append([x_c-1,y_c+1])
        allow_values.append([x_c-1,y_c])
        allow_values.append([x_c,y_c+1])
        # now change these to true values in the mask
        for entry in allow_values:
            mask[entry[0]][entry[1]] = True
        mask_list.append(mask)
        # reset the mask and create the next region
        ...
        # make the first hexagon shape
        allow_values.append([x_c-10,y_c])
        allow_values.append([x_c-9,y_c])
        allow_values.append([x_c-8,y_c])
        allow_values.append([x_c-7,y_c])
        allow_values.append([x_c-6,y_c])
        allow_values.append([x_c-5,y_c])
        allow_values.append([x_c-10,y_c-1])
        allow_values.append([x_c-9,y_c-1])
        allow_values.append([x_c-8,y_c-1])
        allow_values.append([x_c-7,y_c-1])
        allow_values.append([x_c-6,y_c-1])
        allow_values.append([x_c-5,y_c-1])
        allow_values.append([x_c-9,y_c+1])
        allow_values.append([x_c-8,y_c+1])
        allow_values.append([x_c-7,y_c+1])
        allow_values.append([x_c-6,y_c+1])
        allow_values.append([x_c-8,y_c+2])
        allow_values.append([x_c-7,y_c+2])
        allow_values.append([x_c-9,y_c-2])
        allow_values.append([x_c-8,y_c-2])
        allow_values.append([x_c-7,y_c-2])
        allow_values.append([x_c-6,y_c-2])
        allow_values.append([x_c-8,y_c-3])
        allow_values.append([x_c-7,y_c-3])
        # now change these to true values in the mask
        for entry in allow_values:
            mask[entry[0]][entry[1]] = True
        mask_list.append(mask)
        ...
        mask = np.full(shape=(shape_x,shape_y),
                       fill_value=False,
                       dtype=bool)
        # shift everything in the allow values list into the correct
        # position since the mask has the same shape
        for entry in allow_values:
            entry[0] += 4
            entry[1] -= 3
        # now change these to true values in the mask
    ...
    # high signal to noise - use 3x3 square grids across the galaxy
    elif sn_type == 'high':
        # REGION 1 - THE CENTRE
        # set the allow values array - these will be used in conjunction
        # with the mask to set some values equal to TRUE
        ...
        mask = np.full(shape=(shape_x,shape_y),
                       fill_value=False,
                       dtype=bool)
        for i in range(x_c-9,x_c-6):
            for j in range(y_c-4,y_c+1):
                allow_values.append([i,j])
        ...
        mask = np.full(shape=(shape_x,shape_y),
                       fill_value=False,
                       dtype=bool)
        for i in range(x_c-7,x_c-4):
            for j in range(y_c-8,y_c-4):
                allow_values.append([i,j])
        ...
        raise ValueError("CHECK SN TYPE")
...
all_line_flux_file = '/disk2/turner/disk2/turner/DATA/KLP/ANALYSIS/HEXAGON_FITTING/hexagon_fitting_line_fluxes_highsn.txt'
...
with open(all_line_flux_file, 'a') as f:
    ...
    k_centres_x = k_table['Central_x']
    k_centres_y = k_table['Central_y']
    h_table = ascii.read('/disk2/turner/disk2/turner/DATA/KLP/ANALYSIS/Hband/KLP_H_NAMES_FINAL.txt')
    h_names = h_table['Filename']
    h_centres_x = h_table['Central_x']
    h_centres_y = h_table['Central_y']
    yj_table = ascii.read('/disk2/turner/disk2/turner/DATA/KLP/ANALYSIS/YJband/KLP_YJ_NAMES_FINAL.txt')
    yj_names = yj_table['Filename']
    yj_centres_x = yj_table['Central_x']
    yj_centres_y = yj_table['Central_y']
    region_table = ascii.read('/disk2/turner/disk2/turner/DATA/KLP/ANALYSIS/HEXAGON_FITTING/hexagon_list.txt')
    plot_dirs = region_table['PLOT_DIR']

    for k_name,k_centre_x,k_centre_y, h_name,h_centre_x, h_centre_y, yj_name, yj_centre_x, yj_centre_y, redshift, plot_dir in zip(k_names,
                                                                                                                                k_centres_x,
                                                                                                                                k_centres_y,
                                                                                                                                h_names,
                                                                                                                                h_centres_x,
        ...
        all_line_fluxes = fit_all_regions('high',
                                          yj_name,
                                          yj_centre_x,
                                          yj_centre_y,
                                          h_name,
        ...
        for entry in all_line_fluxes:

            gal_name = k_name[len(k_name) - k_name[::-1].find("/"):]
            gal_name = gal_name[26:-5]

            # construct the region name which is the sn_type and the 
            # region number - uniquely identifying where in the galaxy
            # that patch comes from

            region_name = gal_name + '_' + entry[9][0] + '_' + str(entry[9][1])
